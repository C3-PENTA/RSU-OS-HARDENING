...
struct gpio_cfg init_tbl[] = {
	{ TCC_GPB(8)  ,GPIO_FN0 , GPIO_INPUT           , GPIO_PULLDISABLE },	// RST#_IRQ to prevent NAND crash

	{ TCC_GPC(19) ,GPIO_FN0 , GPIO_OUTPUT|GPIO_HIGH, GPIO_PULLDISABLE },	// DRAM0 Power
	{ TCC_GPC(20) ,GPIO_FN0 , GPIO_OUTPUT|GPIO_HIGH, GPIO_PULLDISABLE },	// DRAM1 Power 

	{ TCC_GPF(21) ,GPIO_FN0 , GPIO_OUTPUT|GPIO_HIGH, GPIO_PULLDISABLE },	// BT_RST#
...
void gpio_init_early(void)
{
	...
	if (cpu_is_tcc8963()) {
		for (i=0 ; i<(sizeof(init_tbl_8963)/sizeof(init_tbl_8963[0])) ; i++)
			gpio_config(init_tbl_8963[i].port, init_tbl_8963[i].function|init_tbl_8963[i].direction|init_tbl_8963[i].pull);
	}
	else {
